...
public class AutenticacaoController {
    ...
    public ResponseEntity login(@RequestBody @Valid AutenticacaoDTO autenticacaoDTO) {
        var userPassword = new UsernamePasswordAuthenticationToken(autenticacaoDTO.login(), autenticacaoDTO.password());
    ...
    public ResponseEntity registra(@RequestBody @Valid RegistroDTO registroDTO) {
        if (this.userRepository.findByLogin(registroDTO.login()) != null) {
            return ResponseEntity.badRequest().build();
        }

        String  senhaEncriptada = new BCryptPasswordEncoder().encode(registroDTO.password()); // HASH
        Usuario usuario = new Usuario(registroDTO.login() , senhaEncriptada, registroDTO.role());
